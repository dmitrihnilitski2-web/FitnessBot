...
# --- МОДЕЛІ ДЛЯ СОЦІАЛЬНОГО РУШІЯ (ВІДНОВЛЕНО) ---
...
# =========================================================
# СОЦІАЛЬНИЙ РУШІЙ ТА ГЕЙМІФІКАЦІЯ (ВІДНОВЛЕНО)
# =========================================================

@app.get("/api/leaderboard/global")
async def get_global_leaderboard():
    leaders = await database.get_global_leaderboard()
    return {"status": "success", "data": leaders}


@app.get("/api/leaderboard/team/{trainer_id}")
async def get_team_leaderboard(trainer_id: int):
    leaders = await database.get_team_leaderboard(trainer_id)
    return {"status": "success", "data": leaders}


@app.post("/api/store/buy_freeze")
async def buy_freeze(data: BuyFreezeRequest):
    success = await database.buy_streak_freeze(data.user_id)
    if success:
        return {"status": "success", "message": "Заморозку успішно придбано! ❄️"}
    return {"status": "error", "message": "Недостатньо досвіду (EXP) для покупки."}


@app.post("/api/duels/create")
async def api_create_duel(data: CreateDuelRequest):
    duel_id = await database.create_duel(
        data.initiator_id, data.opponent_id, data.bet_exp, data.duel_type, data.days
    )
...
async def get_duels(user_id: int):
    duels = await database.get_user_duels(user_id)
    return {"status": "success", "data": duels}


# =========================================================
# МАРКЕТИНГ ТА ВІРАЛЬНІСТЬ (ПІДСУМКИ ТРЕНУВАННЯ)
# =========================================================

@app.get("/api/workout_summary/{user_id}")
async def get_workout_summary(user_id: int):
    """
    Збирає статистику поточної сесії (за сьогоднішній день)
    для відображення віральної картки завершення тренування.
    """
    today = datetime.now().strftime("%Y-%m-%d")
    async with aiosqlite.connect(database.DB_NAME) as db:
        db.row_factory = aiosqlite.Row

        # Рахуємо загальний тоннаж (об'єм), кількість підходів та тривалість
        ...
        # Орієнтовні калорії: ~12 ккал за 1 силовий підхід + ~8 ккал за 1 хв кардіо (заглушка)
        estimated_calories = (total_sets * 12) + (total_duration * 8)
        # Орієнтовна тривалість (якщо користувач не писав час, даємо ~3 хв на підхід)
        estimated_minutes = total_duration or (total_sets * 3)

        # Додаємо поточний рівень та вогник
        u_cursor = await db.execute('SELECT level, exp, current_streak FROM users WHERE user_id = ?', (user_id,))
        u_row = await u_cursor.fetchone()
        level = u_row['level'] if u_row else 1
