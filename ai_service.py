...
def extract_json(text: str) -> dict:
    """Надійно витягує JSON з тексту, ігноруючи будь-який маркдаун (наприклад, ```json ... ```)."""
    try:
...
async def generate_workout_plan(user_data: dict, ai_data: dict, cycle_phase: str = "") -> dict:
    ...
    prompt = f"""
    Ти професійний фітнес-тренер, спортивний науковець та реабілітолог. Створи детальний мікроцикл на 1 тиждень.
    ДАНІ КЛІЄНТА:
    ...
    Сміливо використовуй маркдаун (**жирний текст**) для виділення головних думок у текстових описах.
    Тексти (explanation та projections) мають бути розгорнутими, об'ємними та професійними.

    ПОВЕРНИ ВИКЛЮЧНО ВАЛІДНИЙ JSON:
    {{
        "plan_name": "Назва плану",
        "explanation": "Детальне пояснення логіки тренувального циклу, обґрунтування вибору вправ, інтенсивності та методів прогресії (3-5 розгорнутих речень)",
        "projections": "Детальний розрахунковий прогноз фізіологічних змін, покращення силових показників та адаптації тіла за наступні 4 тижні (3-5 розгорнутих речень)",
        "days": [
            {{
                "day": 1,
...
async def adapt_daily_workout(user_data: dict, checkin_data: dict, cycle_phase: str = "") -> dict:
    ...
    femtech_prompt = ""
    if cycle_phase and cycle_phase in ['menstruation', 'luteal']:
        femtech_prompt = f"Зверни увагу: клієнтка у фазі '{cycle_phase}'. Знизь робочі ваги, додай більше розтяжки/мобільності."

    prompt = f"""
    Ти професійний емпатійний тренер. Адаптуй сьогоднішнє тренування клієнта на основі його чек-іну.
    ...
    Якщо стан поганий - зменш підходи/RPE, заміни на кардіо, легку мобільність або розтяжку.
    ВІДПОВІДАЙ ВИКЛЮЧНО ЦІЄЮ МОВОЮ: {lang}. 
    Використовуй маркдаун (**жирний текст**) у повідомленні.

    ПОВЕРНИ ВИКЛЮЧНО ВАЛІДНИЙ JSON:
    {{
        "coach_message": "Розгорнуте, емпатійне та мотиваційне повідомлення від тренера, що детально пояснює причину адаптації плану під поточний стан (3-4 речення)",
        "focus": "Новий фокус (напр. 'Легке відновлення та релаксація')",
