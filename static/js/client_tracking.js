...
function detectExerciseType(name) {
    ...
    // ВИПРАВЛЕНО: Додано маркери для дихальних вправ, медитації, МФР, сканування тіла та йоги
    if (n.includes('розминка') || n.includes('заминка') || n.includes('розтяжка') ||
        ...
        n.includes('йога') || n.includes('мфр') || n.includes('yoga')) {
        return 'warmup';
    }
...
async function loadNutrition() {
    try {
        ...
        if (statusEl) {
            ...
            else if (cals <= goals.calories * 1.1) { statusEl.innerText = loc('nutri_status_perfect', "🟢 Ідеальний баланс"); statusEl.style.color = 'var(--success)'; statusEl.style.background = 'rgba(50, 215, 75, 0.1)'; }
            else { statusEl.innerText = loc('nutri_status_over', "🔴 Перебір калорій"); statusEl.style.color = 'var(--danger)'; statusEl.style.background = 'rgba(255, 69, 58, 0.1)'; }
        }
        ...
        if (logs.length === 0) { logsContainer.innerHTML = `<p style="color: var(--hint-color); font-size: 14px; text-align: center; padding: 20px 0;">${loc('nutri_no_logs', 'Ще немає записів за сьогодні.')}</p>`; }
            ...
            logs.forEach(function(log) {
                ...
                logsContainer.innerHTML += `<div class="food-log-item"><div class="food-log-info"><div class="food-log-title">${log.dish_name}</div><div class="food-log-macros">${weightText}🔥 ${log.calories} ккал | Б: ${log.protein} | Ж: ${log.fats} | В: ${log.carbs}</div></div><div style="display:flex;"><button class="action-btn edit" onclick="openEditFood(${log.id}, '${safeName}', ${log.calories}, ${log.protein}, ${log.fats}, ${log.carbs}, ${log.weight_g || 0})">✏️</button><button class="action-btn delete" onclick="deleteFoodLog(${log.id})">🗑️</button></div></div>`;
            });
        }
    } catch (e) {}
}
...
async function analyzeFoodImage(event) {
    ...
    try {
        ...
        if (labelText) labelText.innerText = loc('btn_photo_food', "📸 Фото страви (ШІ)");
