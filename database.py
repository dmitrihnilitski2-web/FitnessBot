...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        # НОВА ТАБЛИЦЯ: ДУЕЛІ ТА ЗМАГАННЯ
...
# =========================================================
# СОЦІАЛЬНИЙ РУШІЙ ТА ГЕЙМІФІКАЦІЯ (СТРІКИ, ДУЕЛІ, РЕЙТИНГИ)
# =========================================================

async def check_and_update_streak(user_id: int):
    """
    Автоматично нараховує вогники (стріки) за активність.
    Викликається при збереженні тренування, їжі або чек-іну.
    """
    today = datetime.now().strftime("%Y-%m-%d")
    async with aiosqlite.connect(DB_NAME) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute(
            'SELECT current_streak, longest_streak, last_streak_date FROM users WHERE user_id = ?', (user_id,))
        ...
        if last_date == today:
            return  # Вогник за сьогодні вже отримано
        ...
        if last_date:
            ...
            if diff == 1:
                current_streak += 1  # Активність наступного дня
            elif diff > 1:
                # Серія перервана (якщо не було автоматично використано заморозку вночі)
                current_streak = 1
        else:
            current_streak = 1  # Перший день активності
...
async def get_global_leaderboard(limit: int = 100):
    """Повертає глобальний Топ користувачів за EXP"""
    async with aiosqlite.connect(DB_NAME) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute('''SELECT user_id, name, username, level, exp, current_streak
                                 FROM users
                                 WHERE role = "client"
...
async def get_team_leaderboard(trainer_id: int, limit: int = 100):
    """Повертає локальний Топ клієнтів одного тренера"""
    async with aiosqlite.connect(DB_NAME) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute('''SELECT user_id, name, username, level, exp, current_streak
                                 FROM users
                                 WHERE trainer_id = ?
                                   AND role = "client"
                                 ORDER BY exp DESC LIMIT ?''', (trainer_id, limit)) as cursor:
            return [dict(row) for row in await cursor.fetchall()]


async def buy_streak_freeze(user_id: int, cost: int = 500):
    """Купівля 'Заморозки Стріку' за внутрішній досвід (EXP)"""
    async with aiosqlite.connect(DB_NAME) as db:
        db.row_factory = aiosqlite.Row
        ...
        if not row or row['exp'] < cost:
            return False  # Недостатньо досвіду
...
async def create_duel(initiator_id: int, opponent_id: int, bet_exp: int, duel_type: str, days: int):
    """Створення нової дуелі 1 на 1"""
    start_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    end_date = (datetime.now() + timedelta(days=days)).strftime("%Y-%m-%d %H:%M:%S")
    async with aiosqlite.connect(DB_NAME) as db:
        cursor = await db.execute('''INSERT INTO duels
                                     (initiator_id, opponent_id, status, bet_exp, duel_type, start_date, end_date)
                                     VALUES (?, ?, 'pending', ?, ?, ?, ?)''',
                                  (initiator_id, opponent_id, bet_exp, duel_type, start_date, end_date))
...
async def get_duel(duel_id: int):
    """Отримання інформації про конкретну дуель"""
    async with aiosqlite.connect(DB_NAME) as db:
        db.row_factory = aiosqlite.Row
...
async def update_duel_status(duel_id: int, status: str, winner_id: int = None):
    """Оновлення статусу дуелі (прийнята, відхилена, завершена)"""
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute('UPDATE duels SET status = ?, winner_id = ? WHERE id = ?', (status, winner_id, duel_id))
        await db.commit()


async def get_user_duels(user_id: int):
    """Отримання всіх дуелей користувача"""
    async with aiosqlite.connect(DB_NAME) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute('SELECT * FROM duels WHERE initiator_id = ? OR opponent_id = ? ORDER BY id DESC',
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute('''UPDATE users
                            SET name=?,
                                ...
                                language=?
                            WHERE user_id = ?''',
                         (update_data.get('name'), int(update_data.get('age')), int(update_data.get('height')),
                          float(update_data.get('weight')), float(update_data.get('target_weight')),
                          update_data.get('activity_level'), update_data.get('primary_goal'), json.dumps(new_macros),
                          update_data.get('competition_sport', ''), update_data.get('competition_date', ''),
                          update_data.get('cycle_start_date', ''), update_data.get('cycle_length', 28),
                          update_data.get('language', 'uk'), user_id))
...
async def update_user_ai_factors(user_id, ai_data):
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute('''UPDATE users
                         ...
                         (json.dumps(ai_data.get('risk_factors', [])),
                          json.dumps(ai_data.get('exercise_restrictions', [])),
                          json.dumps(ai_data.get('focus_areas', [])), user_id))
        await db.commit()
...
async def log_workout_set(user_id, exercise_name, set_number, weight, reps, exercise_type='strength', duration=0,
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            '''INSERT INTO workout_logs (user_id, date, exercise_name, set_number, weight, reps, exercise_type,
                                         duration, distance, plan_day)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
...
async def log_water(user_id: int, date: str, amount: int):
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        UPDATE SET amount = amount + ?''', (user_id, date, amount, amount))
        await db.commit()
...
async def update_muscle_fatigue_state(user_id: int, muscle: str, fatigue_level: float, update_time: str):
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        UPDATE SET fatigue_level = ?, last_updated = ?''',
